...
_DEFAULT_PROGRAM = [
    ...
    {"vaccine_name": "อหิวาต์สุกร (CSF)", "dose_no": 1, "age_days": 42, "route": "IM", "dose": "1 มล./ตัว", "repeat_days": None, "note": "อายุ 6 สัปดาห์ (IM หรือ SC)"},
    {"vaccine_name": "อหิวาต์สุกร (CSF)", "dose_no": 2, "age_days": 84, "route": "IM", "dose": "1 มล./ตัว", "repeat_days": 365, "note": "อายุ 12 สัปดาห์ แล้วซ้ำทุก 1 ปี"},
]
...
_PIG_KEYWORDS = ("หมู", "สุกร", "ป่วย", "คอก", "ปศุสัตว์", "วัคซีน", "ฉีดยา",
                 "บวม", "หลังฉีด", "ข้างเคียง", "ตรวจอาการ", "ฉีดตรงไหน", "ตำแหน่งฉีด", "วิธีฉีด")


# วิธี/ตำแหน่งฉีดวัคซีน — ความรู้คงที่ชุดเดียวกับหน้า "วิธีฉีดวัคซีน" ของเว็บ (แก้ที่หนึ่งต้องแก้อีกที่)
# ไม่ใส่ขนาดเข็ม/โดส ให้ยึดฉลากวัคซีน กัน AI พูดตัวเลขผิด
_VACCINE_METHOD_INFO = (
    "ความรู้อ้างอิง: ตำแหน่งฉีดวัคซีนสุกรที่ถูกต้อง (ดูภาพได้ที่หน้า 'วิธีฉีดวัคซีน')\n"
    "1) ลูกสุกร ฉีดเข้าใต้ผิวหนัง (SC): บริเวณขาหนีบ/ข้างลำตัวส่วนท้าย จับยกผิวหนังขึ้นเป็นสันแล้วแทงเข็มเอียงเข้าใต้ผิว\n"
    "2) สุกรทั่วไป ฉีดเข้ากล้ามเนื้อ (IM): กล้ามเนื้อคอด้านข้างหลังใบหู แทงเข็มตั้งฉาก ห้ามฉีดสะโพก/ขาหลัง\n"
    "3) สุกรโตเต็มวัย ฉีดเข้าใต้ผิวหนัง (SC): ใต้ผิวหนังด้านคอหลังใบหู ยกผิวแล้วแทงเอียง\n"
    "ทุกแบบ: SC หรือ IM และโดส/ขนาดเข็ม ให้ยึดฉลากวัคซีนหรือสัตวแพทย์ เก็บวัคซีน 2-8°C ตำแหน่งฉีดต้องแห้งสะอาด "
    "ฉีดแล้วบันทึกและตรวจอาการหลังฉีดวันที่ 1, 3, 7\n"
    "วัคซีนอหิวาต์สุกร (Swine Fever Vaccine): เชื้อเป็นชนิดดูดแห้ง ขนาด 10 และ 30 โดส พร้อมน้ำยาละลาย "
    "ฉีดเข้ากล้ามเนื้อหรือใต้ผิวหนัง ตัวละ 1 มล. โปรแกรม: ลูกสุกรจากแม่ที่เคยได้รับวัคซีน เข็ม 1 อายุ 6 สัปดาห์ เข็ม 2 อายุ 12 สัปดาห์ แล้วฉีดซ้ำทุก 1 ปี; "
    "ลูกสุกรจากแม่ที่ไม่เคยได้รับวัคซีน ฉีดเข็มเดียวอายุ 1 วัน และฉีดซ้ำทุก 1 ปี\n"
    "วัคซีนปากและเท้าเปื่อย (FMD): ฉีดเข้ากล้ามเนื้อ ตัวละ 2 มล. เข็ม 1 อายุ 2 เดือน เข็ม 2 อายุ 3 เดือน แล้วฉีดซ้ำทุก 6 เดือน ขนาด 10 และ 75 โดส"
)
_VACCINE_METHOD_WORDS = ("ฉีดตรงไหน", "ฉีดที่ไหน", "ตำแหน่งฉีด", "วิธีฉีด", "ฉีดยังไง", "ฉีดอย่างไร",
                         "ใต้ผิวหนัง", "เข้ากล้าม", "กล้ามเนื้อ", "หลังใบหู", "ขาหนีบ", "sc", "im",
                         "อหิวา", "swine fever", "ปากเท้า", "ปากและเท้า", "fmd", "กี่มล", "กี่โดส", "โปรแกรมวัคซีน", "ฉีดซ้ำ")


def _needs_vaccine_method(text: str) -> bool:
    t = text.lower()
    if any(w in t for w in _VACCINE_METHOD_WORDS):
        return True
    # "ฉีดวัคซีนตรงไหน" — คำว่า ฉีด กับ ตรงไหน อยู่ห่างกัน จับแยกสองส่วน
    return "ฉีด" in t and any(w in t for w in ("ตรงไหน", "ที่ไหน", "ยังไง", "อย่างไร", "วิธี", "ตำแหน่ง", "จุดไหน"))
...
def _build_context(detailed: bool = False, stats_days: Optional[int] = None, pig: bool = False,
    ...
    if pig:
        ...
        if _needs_vaccine_method(_ctx_text):
            parts.append(_VACCINE_METHOD_INFO)
        try:
